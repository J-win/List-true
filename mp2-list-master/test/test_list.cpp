...
{
	List<int> l2;
	List<int> l3 = l.Merge(l.GetHead(), l2);
	EXPECT_EQ(l2, l3);
}
...
{
	EXPECT_EQ(1, l.GetHead()->val);
	EXPECT_EQ(2, l.GetHead()->next->val);
	EXPECT_EQ(3, l.GetHead()->next->next->val);
...
TEST_F(ThreeNodesList, copy_of_not_empty_list_is_correct)
{
	...
	EXPECT_EQ(1, l2.GetHead()->val);
	EXPECT_EQ(2, l2.GetHead()->next->val);
	EXPECT_EQ(3, l2.GetHead()->next->next->val);
...
TEST_F(ThreeNodesList, can_insert_to_head_for_not_empty_list)
{
	l.InsertToHead(5);
	EXPECT_EQ(5, l.GetHead()->val);
	EXPECT_EQ(1, l.GetHead()->next->val);
}

TEST_F(ThreeNodesList, can_insert_to_tail_for_not_empty_list)
{
	l.InsertToTail(5);
	EXPECT_EQ(5, l.GetHead()->next->next->next->val);
...
TEST_F(ThreeNodesList, insert_after_ptr_for_not_empty_list_is_correct)
{
	...
	EXPECT_EQ(2, l.GetHead()->next->val);
	EXPECT_EQ(5, l.GetHead()->next->next->val);
	EXPECT_EQ(3, l.GetHead()->next->next->next->val);
	EXPECT_EQ(NULL, l.GetHead()->next->next->next->next);
}
...
TEST_F(ThreeNodesList, can_delete_middle_from_not_empty_list)
{
	l.Delete(2);
	EXPECT_EQ(1, l.GetHead()->val);
	EXPECT_EQ(3, l.GetHead()->next->val);
}
...
{
	l.Inverse();
	EXPECT_EQ(3, l.GetHead()->val);
	EXPECT_EQ(2, l.GetHead()->next->val);
	EXPECT_EQ(1, l.GetHead()->next->next->val);
	EXPECT_EQ(NULL, l.GetHead()->next->next->next);
}
...
TEST_F(ThreeNodesList, can_assign_two_not_empty_lists_of_eq_size)
{
	...
	l2.InsertToTail(33);
	l = l2;
	EXPECT_EQ(11, l.GetHead()->val);
	EXPECT_EQ(22, l.GetHead()->next->val);
	EXPECT_EQ(33, l.GetHead()->next->next->val);
	EXPECT_EQ(NULL, l.GetHead()->next->next->next);
}
...
{
	List<int> l3 = l.Merge(l.GetHead()->next, l2);
	List<int> res;
	res.InsertToTail(1);
	res.InsertToTail(2);
	res.InsertToTail(11);
...
{
	List<int> l3 = l.Merge(l.GetHead()->next, z);
	EXPECT_EQ(l, l3);
}
